...
if [ ! $BUILD_EXTENSION_ONLY ]; then
    cd theme
    zip -FS -r ../numix-firefox-theme.xpi *
    cd -
fi

if [ ! $BUILD_THEME_ONLY ]; then
    cd extension
    zip -FS -r ../numix-firefox-extension.xpi *
    cd -
fi

if [ ! $BUILD_THEME_ONLY ] && [ ! $BUILD_EXTENSION_ONLY ]; then
    VERSION=$(grep em:version install.rdf | cut -f2 -d'>' | cut -f1 -d'<')
    zip -FS -r numix-firefox-$VERSION.xpi numix-firefox-theme.xpi numix-firefox-extension.xpi install.rdf
fi
